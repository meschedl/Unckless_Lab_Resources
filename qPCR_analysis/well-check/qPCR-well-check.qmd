...
Plot "heatmap style" the counts of times a well has come up with a Cq value lower than 40 for something it should have been 40. I took the spreadsheet above and changed it into counts, so I made one row per well, and did the count of times that well was in the dataset. Then I made new columns that were the row in the plate (1-8 corresponding to A-H) and column in the plate (1-12). The example code I used did the numbering like this for the rows and later changed it to be letter and reordered it to go reverse (A at the top, which was 1). 
Using code to make the plot from this https://rpubs.com/artur_matysik/plot-96well-heatmap 
